...
int main(){
  ...
  //set up the TCPReceiver 
  std::string DistribIP="vmepcs2f17-19";
  //std::string DistribIP="localhost";
  unsigned int errorCode;

  HT.SetIP(DistribIP);
  unsigned int listenPort=51001;
  unsigned int AquireMode=0;
  ...
  //done setting up TCPReceiver


  struct sockaddr_in servAddr;
  struct sockaddr_in clntAddr;
  unsigned int clntLen;
  unsigned short servPort = 51065;
  ...
  while(gContinue){
    ...
    cout << " ** Receiving Lumi Section ** " << endl;
    ...
    //commented out part is for fake data
    /*
    orbitCount += 4;
